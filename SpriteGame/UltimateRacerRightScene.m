//
//  Sprite.m
//  SpriteGame
//
//  Created by Samir Choudhary on 11/8/13.
//  Copyright (c) 2013 Samir Choudhary. All rights reserved.
//

#import "UltimateRacerRightScene.h"

@implementation UltimateRacerRightScene
{
    SKNode* car1;
    SKNode* car2;
    SKShapeNode* track1;
    SKShapeNode* track2;
    BOOL accelerate;
    BOOL pressed;
    BOOL setted;
    SKShapeNode* acceleratorNode1;
    SKShapeNode* acceleratorNode2;
}

-(id)initWithSize:(CGSize)size {
    NSLog(@"Hello");
    
    if (self = [super initWithSize:size]) {
        
        track2 = [SKShapeNode node];
        
        self.backgroundColor = [SKColor blackColor];
        self.physicsWorld.gravity = CGVectorMake(0, 0);
        
        CGRect selfSize = self.frame;
        selfSize.origin.x = (selfSize.size.width/2)*-1;
        selfSize.origin.y = (selfSize.size.height/2 - 112);
        selfSize.size.height -= 700;
        
        track2.path = ([UIBezierPath bezierPathWithRoundedRect:selfSize cornerRadius:10]).CGPath;
        track2.fillColor = [UIColor clearColor];
        track2.strokeColor = [UIColor redColor];
        track2.glowWidth = 7;
        track2.lineWidth = 23;

        car2 = [SKNode node];
        SKShapeNode* circle2 = [SKShapeNode node];
        circle2.path = ([UIBezierPath bezierPathWithOvalInRect:CGRectMake(self.frame.size.width/2-15, self.frame.size.height/2-127, 30, 30)]).CGPath;
        
        car2.physicsBody = [SKPhysicsBody bodyWithCircleOfRadius:15];
        [car2.physicsBody setFriction:0.9];
        [car2.physicsBody setLinearDamping:0.9];
        
        UIColor *myColor2 = [UIColor colorWithRed: 255.0/255.0 green: 193.0/255.0 blue:193.0/255.0 alpha: 1.0];
        circle2.fillColor = myColor2;
        circle2.strokeColor = myColor2;
        [car2 addChild:circle2];
        
        acceleratorNode2 = [SKShapeNode node];
        acceleratorNode2.path = ([UIBezierPath bezierPathWithOvalInRect:CGRectMake(self.frame.size.width/2-50, 100, 100, 100)]).CGPath;
        acceleratorNode2.strokeColor = [UIColor greenColor];
        
        [self addChild:track2];
        [self addChild:car2];
        [self addChild:acceleratorNode2];
        accelerate = NO;
        pressed = NO;

    }
    return self;
}

- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    for (UITouch* touch in touches)
    {
        CGPoint temp = [touch locationInNode:acceleratorNode2];
        if (temp.x > self.frame.size.width/2 - 50 && temp.x < self.frame.size.width/2 + 50 && temp.y < 200  && temp.y > 100) {
            acceleratorNode2.fillColor = [UIColor yellowColor];
            acceleratorNode2.glowWidth = 20;
            accelerate = YES;
            pressed = YES;
        }
    }
    
    acceleratorNode1.fillColor = [UIColor yellowColor];
     acceleratorNode1.glowWidth = 20;
     
     accelerate = YES;
     pressed = YES;
}

- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    acceleratorNode2.fillColor = [UIColor clearColor];
    acceleratorNode2.glowWidth = 0;
    
    accelerate = NO;
    pressed = NO;
}

-(void)update:(CFTimeInterval)currentTime {
    /* Called before each frame is rendered */
    
    CGFloat limit = 384;
    CGFloat posx = car2.position.x;
    CGFloat posy = car2.position.y;
    //NSLog(@"Look%f",posx);
    
    if (posx - limit > 0.01 && setted == NO)
    {
        
        setted = YES;
        CGFloat refx = car1.physicsBody.velocity.dx * -1;
        
        car2 = [SKNode node];
        SKShapeNode* circle2 = [SKShapeNode node];
        circle2.path = ([UIBezierPath bezierPathWithOvalInRect:CGRectMake(768, posx+324, 30, 30)]).CGPath;
        UIColor *myColor1 = [UIColor colorWithRed: 176.0/255.0 green: 226.0/255.0 blue:255.0/255.0 alpha: 1.0];
        circle2.fillColor = myColor1;
        [car2 addChild:circle2];
        
        car2.physicsBody = [SKPhysicsBody bodyWithCircleOfRadius:15];
        [car2.physicsBody setFriction:0.9];
        [car2.physicsBody setLinearDamping:0.9];
        [self addChild:car2];
        
        CGVector trial = CGVectorMake(18, 0);
        [car2.physicsBody setVelocity:CGVectorMake(refx, 0)];
        [car2.physicsBody applyForce:trial];
    }
    
    else if (posx - limit > 0.01 && accelerate && pressed)
    {
        CGVector trial = CGVectorMake(0, 18);
        [car2.physicsBody applyForce:trial];
    }
    
    else if (accelerate && pressed) {
        if ( setted == NO) {
            CGVector trial = CGVectorMake(-18, 0);
            [car2.physicsBody applyForce:trial];
        }
        else {
            CGVector trial = CGVectorMake(-18, 0);
            [car2.physicsBody applyForce:trial];
        }
    }
    else if ( accelerate && pressed && posy){
        
    }
    
    else {
        CGVector trial = CGVectorMake(0, 0);
        [car2.physicsBody applyForce:trial];
    }
}




@end
